...
const CARD_PALETTES = [
  {
    bg: "from-blue-100 to-blue-50 border-blue-300",
    op: "text-blue-600",
    result: "bg-emerald-50 border-emerald-300 text-emerald-700",
  },
  {
    bg: "from-purple-100 to-purple-50 border-purple-300",
    op: "text-purple-600",
    result: "bg-emerald-50 border-emerald-300 text-emerald-700",
  },
  {
    bg: "from-orange-100 to-orange-50 border-orange-300",
    op: "text-orange-600",
    result: "bg-emerald-50 border-emerald-300 text-emerald-700",
  },
  {
    bg: "from-cyan-100 to-cyan-50 border-cyan-300",
    op: "text-teal-600",
    result: "bg-emerald-50 border-emerald-300 text-emerald-700",
  },
  {
    bg: "from-pink-100 to-pink-50 border-pink-300",
    op: "text-rose-600",
    result: "bg-emerald-50 border-emerald-300 text-emerald-700",
  },
  {
    bg: "from-amber-100 to-amber-50 border-amber-300",
    op: "text-amber-700",
    result: "bg-emerald-50 border-emerald-300 text-emerald-700",
  },
];
        ...
        return (
          ...
          >
            <div className="flex items-center justify-center gap-1.5 flex-wrap text-base leading-snug mb-2 min-h-[2.5rem]">
              <span className="bg-white/80 rounded-xl px-2.5 py-1.5 text-lg font-medium">
                {ex.leftLabel}
              </span>
              <span className={`font-black text-xl ${pal.op} px-1`}>
                {ex.op}
              </span>
              <span className="bg-white/80 rounded-xl px-2.5 py-1.5 text-lg font-medium">
                {ex.rightLabel}
              ...
              {ex.resultLabel && (
                <>
                  ...
                  <span
                    className={`rounded-xl px-2.5 py-1.5 text-lg font-bold border-2 ${pal.result}`}
                  >
                    {ex.resultLabel}
                  </span>
                </>
            ...
            <p className="text-center text-xs font-bold text-slate-600 leading-tight">
              {ex.legend}
            </p>
          </motion.div>
        );
      ...
      {realSteps.map((step, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-2">
            ...
            >
              <span className="text-3xl leading-tight text-center">
                {step.icon}
              </span>
              {step.label && (
        ...
        <CardContent className="p-4 space-y-2">
          <div className="grid grid-cols-1 gap-2">
            {challenge.options.map((opt, i) => {
              ...
              let cls =
                "w-full text-left px-4 py-2.5 rounded-xl border-2 text-sm font-medium transition-all duration-200 ";
              if (!answered) {
                cls +=
...
export default function LessonPage() {
  ...
  const { data: lessonProgress, refetch: refetchLessonProgress } = useQuery({
    queryKey: ["lesson-progress", lessonId],
    queryFn: () => api.progress.getLesson(lessonId!),
    enabled: !!lessonId,
  });

  useEffect(() => {
    if ((lessonProgress as any)?.completed) {
  ...
  const progressMutation = useMutation({
    mutationFn: () =>
      api.progress.update(courseId!, lessonId!, { completed: true }),
    onSuccess: () => {
  ...
  const quizProgressMutation = useMutation({
    mutationFn: ({ score, total }: { score: number; total: number }) =>
      api.progress.update(courseId!, lessonId!, {
        quizCompleted: true,
        quizScore: score,
      }),
    onSuccess: () => refetchLessonProgress(),
  });

  const experimentProgressMutation = useMutation({
    mutationFn: () =>
      api.progress.update(courseId!, lessonId!, { experimentCompleted: true }),
    onSuccess: () => refetchLessonProgress(),
  });

  const lesson = useMemo(
    () => (course as any)?.lessons?.find((l: any) => l.id === lessonId),
    [course, lessonId],
  );
  const lessons = useMemo(() => (course as any)?.lessons ?? [], [course]);
  const currentIndex = useMemo(
    () => lessons.findIndex((l: any) => l.id === lessonId),
    [lessons, lessonId],
  );
  const nextLesson = lessons[currentIndex + 1];
  const prevLesson = lessons[currentIndex - 1];

  const challenges: ChallengeItem[] = useMemo(
    () => lesson?.challenges ?? [],
    [lesson],
  );
  ...
  function handleExperimentComplete() {
    if (!(lessonProgress as any)?.experimentCompleted) {
  ...
  async function handleComplete() {
    ...
    confetti({
      ...
      spread: 120,
      origin: { y: 0.5 },
      colors: ["#0DA2E7", "#26d0ce", "#fbbf24", "#f472b6"],
    });
    toast({
      title: "Lesson Complete! 🎉",
      description: "Excellent work! Keep going!",
    });
  }
  ...
  return (
    <Layout>
      <div ref={topRef} className="max-w-3xl mx-auto space-y-6 pb-12">
        {/* Breadcrumb */}
              ...
              <div className="flex items-center gap-3 px-5 py-4 border-b border-border/40 bg-gradient-to-r from-violet-50 to-purple-50">
                ...
                <div>
                  <h2 className="font-extrabold text-foreground">
                    {media.layout === "combine"
                      ? "See It in Action — Visual Examples"
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-0.5">
                  {totalChallenges} questions to test your understanding
                </p>
              </div>
              {answeredCount > 0 && (
                <div className="text-right">
                  <div className="text-2xl font-extrabold text-primary">
                    {challengeScore}/{answeredCount}
                  </div>
            ...
            {answeredCount > 0 && (
              <div className="w-full bg-secondary rounded-full h-2">
                <motion.div
                  ...
                  initial={{ width: 0 }}
                  animate={{
                    width: `${(answeredCount / totalChallenges) * 100}%`,
                  }}
        ...
        >
          {!showQuiz ? (
            (lessonProgress as any)?.quizCompleted ? (
              <Card className="border-2 border-green-200 bg-green-50">
                <CardContent className="p-6 flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-green-400 to-emerald-500 flex items-center justify-center text-2xl flex-shrink-0 shadow-md">
                    ✅
                  </div>
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-green-700">
                      Score:{" "}
                      <strong>
                        {(lessonProgress as any).quizScore ?? 0}/5
                      </strong>{" "}
                      — Great job!
                    </p>
                  </div>
              ...
              <Card
                className="border-dashed border-2 border-primary/30 bg-primary/5 hover:bg-primary/10 transition-colors cursor-pointer"
                onClick={() => setShowQuiz(true)}
              >
                <CardContent className="p-6 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      5 unique AI-generated questions about {lesson.title}
                    </p>
                  </div>
            ...
            <AIQuizExperiment
              ...
              onComplete={(score, total) =>
                quizProgressMutation.mutate({ score, total })
              }
            />
          )}
        </motion.div>
          ...
          {!lessonComplete ? (
            ...
            <div className="flex items-center gap-3">
              ...
              {nextLesson ? (
                ...
                >
                  Next:{" "}
                  {nextLesson.title.length > 22
                    ? nextLesson.title.slice(0, 22) + "…"
                    : nextLesson.title}{" "}
                  →
                </Button>
              ) : (
